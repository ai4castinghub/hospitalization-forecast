...
process_disease_flatline_forecaster <- function(disease, data, forecast_date) {
  ...
  weekly_canned_results <- lapply(ahead_seq, function(days_ahead) {
    flatline_forecaster(
      data_hist,
      outcome = disease,
...
run_for_forecast_date <- function(forecast_date) {
  
  print(paste("Running for:", forecast_date))
  
  all_preds <- bind_rows(
    lapply(c("covid", "rsv", "flu"),
...
lapply(forecast_dates, run_for_forecast_date)